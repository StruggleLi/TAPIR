...
//TODO
/*
(1)需要添加静态数据流分析的内容，实现在正确的地点进行插桩。
 */
//每修改一次代码，要记住重新编译所有修改过的class+rebuild项目
//这里后续需要添加一些options的参数，进而可以重复利用已有的代码（不懂这里在说什么）
...
public class MyInstrumenter {
    public static String androidJar="H:\\PALOMA-Analysis\\PALOMA-Analysis\\lib\\";
    //OKd的APK：nori.apk|fr.neamar.kiss_146.apk（已经尝试）|androFotoFinder.apk|passandroid_349.apk(已经尝试)|mpdroid_58.apk
    //被混淆的APK：nori.apk
    ...
    public static String APK="H:\\TestApk\\318APhotoM.apk";//318APhotoM.apk;//Application-release.apk";
...
*/
        ...
        PackManager.v().getPack("jtp").add(new Transform("jtp.MyTransform", new yixue.icse.MyTransform()));//添加自己的BodyTransformer
...
class MyTransform extends BodyTransformer{
    ...
    protected void internalTransform(Body body, String arg1,
        ...
        //here, we get each body one by one
        System.out.println("Enter internalTransform");
        String P_className="GalleryCursorAdapterFromArray";
        String P_methodName="getView";//"getView";
        String P_APICall="setImageURI";//"setImageURI";

        String P_className2="HugeImageLoader";
        String P_methodName2="loadImage";//"getView";
        String P_APICall2="decodeFile";//"setImageURI";
        int P_lineNumber=99;

        //used for image resource replacement
        String R_className="ImagePagerAdapterFromCursorArray";
        String R_methodName="instantiateItem";//"getView";
        String R_APICall="getFullFilePathfromArray";//"setImageURI";

        String R_className2="ImagePagerAdapterFromCursor";
        String R_methodName2="instantiateItem";//"getView";
        String R_APICall2="getFullFilePath";//"setImageURI";

        String R_className3="GalleryCursorAdapter";
        String R_methodName3="bindView";//"getView";
        String R_APICall3="getString";//"setImageURI";


        //另外几个需要插桩的地方----似乎这个方法是被公用的
        //ImagePagerAdapterFromCursor.java    setImageFromThumbnail()     ImageLoader.getInstance().displayImage("file://" + imageFile, photoView, mDisplayImageOptions);
        //ThumbNailUtils.java    displayImageImpl()     ImageLoader.getInstance().displayImage(uriString, imageView, mDisplayImageOptions);

        String P_className4="ImagePagerAdapterFromCursor";
        String P_methodName4="setImageFromThumbnail";//"getView";
        String P_APICall4="displayImage";//"setImageURI";

        String P_className5="ThumbNailUtils";
        String P_methodName5="displayImageImpl";//"getView";
        String P_APICall5="displayImage";//"setImageURI";

        //Function 1:   insert the statement:  Log.i("test",toast);--------这里用来插入opsition信息，插桩了哪个Decoding API
        //InsertLog(body,P_className,P_methodName,P_APICall,P_lineNumber);//success
        InsertDecodingLog2(body,P_className,P_methodName,P_APICall,P_lineNumber);//success--改造完成
        InsertDecodingLog3(body,P_className2,P_methodName2,P_APICall2,P_lineNumber);//success--改造完成-----这个是处理存在两个decodeFile的情况
        InsertDecodingLog2(body,P_className4,P_methodName4,P_APICall4,P_lineNumber);//success--改造完成
        InsertDecodingLog2(body,P_className5,P_methodName5,P_APICall5,P_lineNumber);//success--改造完成

        //Function 2:   insert the statement:  System.out.println(“HELLO”)
        //InsertPrintln(body,P_methodName,P_APICall,P_lineNumber);//success
        //InsertPrintln2(body,P_methodName2,P_APICall2,P_lineNumber);//success

        //insert the statement:  System.out.println(“location”)
        //InsertPrintlnLoation(body,P_methodName,P_APICall,P_lineNumber);//success
        //InsertPrintlnLoation2(body,P_methodName2,P_APICall2,P_lineNumber);//success

        //Function 3:   insert time stamp
        //InsertTimeStamp(body,P_methodName,P_APICall,P_lineNumber);//success
        InsertTimeStamp2(body,P_className,P_methodName,P_APICall,P_lineNumber);//success
        InsertTimeStamp3(body,P_className2,P_methodName2,P_APICall2,P_lineNumber);//success
        InsertTimeStamp2(body,P_className4,P_methodName4,P_APICall4,P_lineNumber);//success
        InsertTimeStamp2(body,P_className5,P_methodName5,P_APICall5,P_lineNumber);//success

        //Function 4:   创建赋值语句并替换参数，这里既可以替换方法调用的参数，也可以替换赋值语句的右值
        //实现了语句的插入：String str = Proxy.getUrlStr()；Proxy.getUrlStr();;和一个信息输出语句
        //InsertAssignment3(body,P_methodName,P_APICall,P_lineNumber);//success


        //在一条赋值语句的右侧替换为我们需要的URL获取AP，后面可以扩展为获取其它不同的资源
        replaceRight(body,R_className,R_methodName,R_APICall);//success
        replaceRight(body,R_className2,R_methodName2,R_APICall2);//success
        replaceRight(body,R_className3,R_methodName3,R_APICall3);//success-----如果不插桩这个，就能在打开相册的时候，显示所有的略缩图
        //ReplaceParameter(body,P_methodName,P_APICall,P_lineNumber);

        //Function 4:   替换参数，这里既可以替换方法调用的参数，也可以替换赋值语句的右值
        //ReplaceParameter(body,P_methodName,P_APICall,P_lineNumber);

        //Function 5:   insert assignment statement    先新建一个字符变量,并赋值一个常量
        //InsertAssignment(body,P_methodName,P_APICall,P_lineNumber);

        //Function 6:   insert memory usage println statement
        //InsertTimeStamp1(body,P_methodName,P_APICall,P_lineNumber);//success,输出的是当前内存消耗的两倍大小。为什么其他的没有通过呢？这是一个需要进一步分析的问题
        InsertMemoryStamp3(body,P_className,P_methodName,P_APICall,P_lineNumber);
        InsertMemoryStamp4(body,P_className2,P_methodName2,P_APICall2,P_lineNumber);//success,输出的是当前内存消耗的两倍大小。为什么其他的没有通过呢？这是一个需要进一步分析的问题
        InsertMemoryStamp3(body,P_className4,P_methodName4,P_APICall4,P_lineNumber);
        InsertMemoryStamp3(body,P_className5,P_methodName5,P_APICall5,P_lineNumber);
    ...
    public void InsertDecodingLog2(Body body,String P_className,String P_methodName,String P_APICall,int P_lineNumber){
        String tag="InsertDecodingLog2";
        String className=body.getMethod().getSignature();
        System.out.println("The className is:   "+body.getMethod().getDeclaringClass());
        String methodName=body.getMethod().getName();
        Iterator<Unit> unitsIterator=body.getUnits().snapshotIterator();
    ...
    public void InsertTimeStamp2(Body body,String P_className,String P_methodName,String P_APICall,int P_lineNumber){
        String className=body.getMethod().getSignature();
        String methodName=body.getMethod().getName();
        final PatchingChain<Unit> units = body.getUnits();
        Iterator<Unit> unitsIterator=units.snapshotIterator();
...
//		"getDrawableForDensity("
        ...
        if (stmt.containsInvokeExpr()) {
            ...
            for(int i=0;i<Bitmap_method.length;i++){
                if(declaringClass.contains(Bitmap_method[i])){
                    checkResult=true;
                }
            }
        }
...
//判断是否是以On开头
    public boolean checkCallbackMethod(SootMethod sootMethod){
        boolean checkResult=false;
        System.out.println("Checking sootMethod.getName():"+sootMethod.getName());
